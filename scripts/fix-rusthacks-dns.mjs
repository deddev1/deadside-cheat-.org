/**
 * Attach deadsidecheat.org to Worker islecheat and create DNS CNAMEs.
...
const DOMAINS = ['deadsidecheat.org', 'www.deadsidecheat.org'];
...
async function ensureCname(name, content) {
	const fqdn = name.includes('.') ? name : `${name}.deadsidecheat.org`;
	const existing = await listDns();
	const hit = existing.find(
		(r) => r.name === fqdn || r.name === name || (name === '@' && r.name === 'deadsidecheat.org'),
	);
	if (hit) {
	...
	const recordName = name === 'deadsidecheat.org' || name === '@' ? 'deadsidecheat.org' : name;
...
async function main() {
	...
	await ensureCname('deadsidecheat.org', TARGET);
